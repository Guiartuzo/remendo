...
/// The language key for a path, by extension. `None` for extensions we do not
/// map. Only the languages with a bundled grammar are listed — vybim's copy of
/// this map also carried keys it served over LSP but never highlighted, which
/// would be dead entries here.
///
/// ```
/// # use std::path::Path;
/// # use remendo::syntax::language_of;
/// assert_eq!(language_of(Path::new("a/b.rs")), Some("rust"));
/// assert_eq!(language_of(Path::new("README.md")), None);
/// ```
...
impl Syntax {
    ...
    /// ```no_run
    /// # use std::path::Path;
    /// # use remendo::syntax::Syntax;
    /// let syntax = Syntax::for_path(Path::new("src/main.rs"));
    /// assert!(syntax.is_some());
    /// ```
    ///
    /// In vybim this delegated to the LSP registry's `language_of`, so the
    /// highlighter and the language servers could not drift apart. Remendo has
    /// no LSP, so the map lives here as the only consumer.
    pub fn for_path(path: &Path) -> Option<Syntax> {
        Self::for_language(language_of(path)?)
    }
