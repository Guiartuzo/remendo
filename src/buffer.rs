...
impl Buffer {
    ...
    ///
    /// ```
    /// # use remendo::buffer::Buffer;
    /// let b = Buffer::from_text("hello\nworld\n");
    /// // 3, not 2: the trailing newline yields a final empty line, matching
    /// // how editors display files. See `line_count`.
    /// assert_eq!(b.line_count(), 3);
    /// assert_eq!(b.line_text(1), "world");
    /// ```
    ///
    /// DIVERGENCE from vybim, where this is `from_str`. There it sits in a
    /// binary crate, so it is not public API and clippy's `should_implement_trait`
    /// stays quiet; here the module is exported from a library, and a
    /// `from_str` that is not `FromStr::from_str` is exactly the confusion that
...
mod tests {
    ...
    fn undo_redo_empty_stacks_are_none() {
        let mut b = Buffer::from_text("seed");
